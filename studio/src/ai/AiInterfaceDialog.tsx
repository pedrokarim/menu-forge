...
import { renderGenerator } from '../model/generator';
...
function MenuSketch({ menu }: { menu: MenuDefinition }) {
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    void Promise.all(
      menu.layers.map((layer) => (layer.generator ? Promise.resolve(renderGenerator(layer.generator, layer)) : loadImage(textureUrl(layer.texture, 0)))),
    ).then((images) => {
      if (cancelled) return;
