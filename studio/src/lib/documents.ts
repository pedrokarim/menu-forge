...
/** Nom du type de document, avec son article (« le menu », « l’image »). */
export const DOCUMENT_NOUNS: Record<DocumentType, string> = { menu: 'le menu', asset: 'l’asset', pixel: 'l’image' };
...
export async function trashWithConfirmation(
  ...
  const question =
    `Mettre ${DOCUMENT_NOUNS[type]} « ${name} » à la corbeille${NBSP}? ` +
    `Le document est déplacé dans le dossier .trash de l’espace de travail${NBSP}: rien n’est supprimé.`;
